...
@endphp
    ...
    <div class="mt-3">
        <form method="get" action="{{ route('rankings.monthly') }}" id="select_target">
            <div class="form-row">
                <div class="col-md-4 mb-3">
                    <select class="custom-select" name="select_monthlyrank" id="type_date" form="select_target">
                        <option value="">対象とするランキングを選んで下さい</option>
                        <option value="0" {{$monthlyrank == 0 ? 'selected' : ''}}>自動更新しない</option>
                        <option value="21" {{$monthlyrank == 21 ? 'selected' : ''}}>初回リピート数</option>
                        <option value="22" {{$monthlyrank == 22 ? 'selected' : ''}}>売上高</option>
                        <option value="23" {{$monthlyrank == 23 ? 'selected' : ''}}>鑑定回数</option>
                    </select>
                </div>
                <div class="col-md-4 mb-3">
...
@if ($monthlyrank != 0)
        <form method="get" action="{{ route('rankings.monthly') }}" id="monthly" name="monthly">
            <div class="col-md-4">
                <div class="mb-3">
                    <span>CSV (ID)</span>
                </div>
                ...
                <div class="col-md-4 mb-3">
                    <button type="submit" class="btn btn-primary mb-2" form="monthly">確認</button>
                </div>
            </div>
        </form>

        <div class="col-md-4" id="areaOneDate">
            <form method="get" action="{{ route('rankings.monthlyUpdate') }}" id="monthly-update" name="monthly-update">
                ...
                <div class="col-md-4 mb-3">
                    <button type="submit" class="btn btn-primary mb-2" form="monthly-update">更新</button>
                </div>
